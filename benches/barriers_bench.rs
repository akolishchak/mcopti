...
use mcopti::{
    Config, Context, DEFAULT_CONFIG, EntryBarriers, LegUniverse, OptionType, Position, Scenario,
...
fn bench_entry_barriers_new(c: &mut Criterion) {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    ensure_market_data_db(&manifest_dir);
    let (context, universe) = build_case(2, &manifest_dir);
    let scenario = Scenario::new(&context, &universe).expect("failed to build scenario");
    let position = &universe.positions[0];

    c.bench_function("entry_barriers_new_call_credit_spread", |b| {
        b.iter(|| {
            let barriers = EntryBarriers::new(
                black_box(&context),
                black_box(position),
                black_box(&scenario),
                black_box(1.0),
...
criterion_group! {
    ...
    targets = bench_entry_barriers_new
}
