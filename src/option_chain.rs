...
pub fn parse_option_chain_file(path: impl AsRef<Path>) -> Result<OptionChain, OptionChainError> {
    let buf = std::fs::read(path)?;
    let chain: OptionChain = serde_json::from_slice(&buf)?;
    Ok(chain)
}
...
pub fn parse_option_chain_reader<R: std::io::Read>(
    reader: R,
) -> Result<OptionChain, serde_json::Error> {
